  ...
  with (
    ...
    then {
      ...
      buildFrontendVersion =
        "2."
        + toString (builtins.fromJSON (lib.versions.minor buildCompiler.version) + 70)
        + ".1";
    }
    else throw ("Unrecognised build compiler " + buildCompiler.name)
  ); let
